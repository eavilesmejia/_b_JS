...
Form = function () {
    ...
    _proto.fail = function (field, error) {
        var self = this,
            _notify = {
                field: field,
                error: error,
                coords: _.cartesian_plane(field)
            };
    ...
    _proto.pack = function (form) {
        ...
        while (x--) {
            ...
            if (!(__.$(field).data('skip')) && _.is_empty(fieldValue)) {
                self.fail(field, 'empty');
                break;
            } else {
                if (__.$(field).data('mail') && !_.is_mail(fieldValue)) {
                    self.fail(field, 'invalid_mail');
                    break
                } else {
                    if (__.$(field).data('max') && (+__.$(field).data('max') > fieldValue.length)) {
                        self.fail(field, 'overflow_chars');
                        break;
                    } else {
                        if (__.$(field).data('custom')) {
                            var Regex = new RegExp(__.$(field).data('custom'), "g");
