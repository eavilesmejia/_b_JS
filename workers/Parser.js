...
addEventListener('message', function (e) {
    ...
    var data = e.data,
        ...
        Template = {
            ...
            repeat_string: function (str, times) {
            ...
            for_loop: function (loop, fields, parent) {
                ...
                if (_self.object_as_string(loop) === '[object String]') {
                    loop = loop.trim();
                    loop = loop.split('\n')
                }
                ...
                if (_vars_split[3].indexOf('-') > -1)
                    _filter = _self.to_object(_vars_split[3].split('-'));

                response = !response ? loop[1] : response;
                _tmp_filter = _filter;

                while (_i--) {
                    //console.log(_keys[_i] + ' is ' + _self.object_as_string(fields[_keys[_i]]) + ' and value = ' + fields[_keys[_i]])

                    var _type = _self.object_as_string(fields[_keys[_i]]);
                    if ((_type === '[object Object]' || _type === '[object Array]')) {
                        ...
                        if (!parent && _length > 1) {
                            response = _self.repeat_string(response, _length);
                        }
